...
export default function Topbar({ title, eyebrow = 'MONTHLY PROCUREMENT CONTROL' }: { title: string; eyebrow?: string }) {
  return <header className="topbar">
    ...
    <div className="topbar-tools"><nav className="top-nav" aria-label="보조 메뉴"><span className="top-nav-link active">전체 현황</span><span className="top-nav-link">지표</span><span className="top-nav-link">인사이트</span></nav><div className="top-meta"><span className="local-badge">SUPABASE LIVE</span><span>기준월도 <b>2026.09</b></span></div></div>
  </header>;
}
